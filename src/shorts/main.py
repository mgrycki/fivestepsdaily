...
from .. import config, generate, limits, storage
from . import heygen
...
def main() -> int:
    ...
    body = f"{source['title']}\n\n{source['body']}"
    caps = {
        "instagram": limits.for_instagram(body, source["hashtags"]),
        "facebook": limits.for_facebook(body, source["hashtags"]),
        "youtube": limits.compose(body, source["hashtags"], 5000),
